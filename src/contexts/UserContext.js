...
const UserProvider = ({children}) => {
  ...
  const getDiaryForm02b_PLIIb = async () => {
    try {
      if (await Storage.getItem('token')) {
        const response = await instance.get('api/FormAppendix/thongtinvantai/getall');
        return response.data;
      }
    } catch (error) {
      if (error.response.status === 401) {
  ...
  const postForm02b_PLIIb = async obj => {
    try {
      setIsLoading(true);
      const response = await instance.post('api/FormAppendix/thongtinvantai/create', obj);
  ...
  const getDiaryForm04_PLIII_03 = async () => {
    try {
      if (await Storage.getItem('token')) {
        const response = await instance.get('api/FormAppendix/xacnhancamket/getall');
        return response.data;
      }
    } catch (error) {
      if (error.response.status === 401) {
  ...
  const postForm04_PLIII_03 = async obj => {
    try {
      setIsLoading(true);
      const response = await instance.post('api/FormAppendix/xacnhancamket/create', obj);

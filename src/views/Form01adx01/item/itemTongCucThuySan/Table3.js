...
const Table3 = ({changeNumber, departurePort, arrivalPort, diary}) => {
  ...
  const handleChuyenBienSo = value => {
    setInputValue({...inputValue, changeNumber: value});
    setThongTinTau({...thongTinTau, chuyenbien_so: value});
  };
  const handleCangDi = value => {
    setInputValue({
      ...inputValue,
      departurePort: {...departurePort, text: value},
    });
  ...
  const handleNgayDi = value => {
    handleDateChange('departurePort', value);
    setThongTinTau({...thongTinTau, ngay_di: dateNowFormat(value, 'date')});
  };
